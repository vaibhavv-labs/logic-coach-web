      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.totalAttempted > 0) {
            fetchedLeaders.push({
              id: doc.id,
              username: data.roadmap?.username || "Anonymous Developer",
      ...
      <div className="modal-content auth-modal" style={{ maxWidth: '500px' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '400px', overflowY: 'auto', paddingRight: '8px' }}>
            {leaders.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>
                No active users yet. Be the first to start solving!
              </div>
            ) : (
              leaders.map((user, idx) => (
                <div key={user.id} style={{ 
                  ...
                  gap: '16px',
                  padding: '12px 16px', 
                  background: idx === 0 ? 'var(--accent-orange-light)' : (idx === 1 ? 'var(--bg-card)' : 'var(--bg-secondary)'),
                  border: idx === 0 ? '1px solid var(--accent-orange)' : '1px solid var(--border-light)',
                  borderRadius: 'var(--radius-lg)'
                }}>
                  <div style={{ 
                    width: '32px', height: '32px', borderRadius: '50%', 
                    background: idx === 0 ? '#fbbf24' : (idx === 1 ? '#9ca3af' : (idx === 2 ? '#b45309' : 'var(--bg-subtle)')),
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 'bold', color: 'var(--accent-teal)' }}>{user.totalAttempted} solved</div>
